...
from server.app.db.models import RoleModel, PermissionModel, SystemConfigModel
...
def _seed_database():
    """Seed initial roles, permissions, and system config if empty."""
    db = SessionLocal()
    try:
        # Seed permissions
        ...
        db.rollback()
        logger.error("Error seeding database defaults: %s", e)
    finally:
        db.close()
...
# 2. Security headers, rate limiting, CSRF
...
async def custom_401_handler(request: Request, exc):
    return JSONResponse(
        ...
        content={"detail": getattr(exc, "detail", "Unauthorized access.")},
...
async def custom_403_handler(request: Request, exc):
    return JSONResponse(
        ...
        content={"detail": getattr(exc, "detail", "Forbidden. Insufficient RBAC permissions.")},
    )
